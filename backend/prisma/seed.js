...
const products = [
    ...
    },
    {
        ...
        price: 99.0,
        inventory: 21,
        imageUrl: "https://images.unsplash.com/photo-1496171367470-9ed9a91ea931?auto=format&fit=crop&w=900&q=80",
        featured: false,
        status: ProductStatus.ACTIVE,
        categoryName: "Home Studio",
    },
...
async function main() {
    ...
    const createdCategories = await Promise.all(categories.map((category) => prisma.category.create({
        data: category,
    })));
    const categoryMap = new Map(createdCategories.map((category) => [category.categoryName, category.categoryId]));
